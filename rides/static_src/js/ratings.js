...
(function () {
  ...
  async function initWidget(widget) {
    ...
    if (voted) {
      ...
      if (!turnstile || !turnstileEl) {
        ...
        turnstileWidgetId = turnstile.render(turnstileEl, {
          ...
          callback(token) {
            currentToken = token;
            setStatus(statusEl, "Choisissez une note.", "");
          },
          "expired-callback"() {
            currentToken = "";
            setStatus(statusEl, "Validation expirée. Rechargez le contrôle avant de voter.", "error");
          },
          "error-callback"() {
            currentToken = "";
    ...
    async function submitVote(rating) {
      ...
      if (!currentToken) {
        setStatus(statusEl, "Complétez la validation anti-robot avant de voter.", "error");
        return;
      }
